...
function error_handle() {
  ...
  clearInterval(myTimer);
  if (restartCount<=2) {
    message.innerHTML = "Get still error. <br>Restart "+restartCount+" times.";
    myTimer = setInterval(function(){colorDetect.click();},10000);
    ifr.src = document.location.origin+'?restart';
  }
  else
    message.innerHTML = "Get still error. <br>Please close the page and check ESP32C.";
}    
...
cv.cvtColor(orig, dst, cv.COLOR_RGBA2GRAY, 0);
cv.imshow('grayOutput', dst);
//-------------------------------------------------
  ...
  Gray = ((R*0.3)+(G*0.59)+(B*0.11)); //Gray
  console.log("RDATA = " + R);
  console.log("GDATA = " + G);
  console.log("BDATA = " + B);
  console.log("ADATA = " + A);
  console.log("GrayDATA = " + Gray); //Gray
  drawRGB_PROBE_Text();
